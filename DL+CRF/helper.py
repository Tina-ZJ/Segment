...
def getTest_New(test_path, char2id, seq_max_len, batch_size):
    predict_idx = []
    predict_string = []
    count = 0
    newchar = char2id["<NEW>"]
    ...
    for line in open(test_path,'r'):
        ...
        terms = (list(line))
        for term in terms:
            charidx.append(char2id[term] if term in char2id else newchar)
        predict_idx.append(charidx)
        predict_string.append(line)
        if count % batch_size==0:
            yield (predict_idx, predict_string)
            predict_idx, predict_string=[], []


def getTrain_New(train_path, char2id, label2id, seq_max_len, batch_size):
    x_train = []
    y_train = []
    count = 0
    newchar = char2id["<NEW>"]
    ...
    for line in open(train_path,'r'):
        ...
        for term in terms:
            ...
            if idx >=0:
                c = term[:idx]
                l = term[idx+1:]
                charidx.append(char2id[c] if c in char2id else newchar)
                labelidx.append(label2id[l] if l in label2id else padlabel)
        x_train.append(charidx)
        y_train.append(labelidx)
        if count % batch_size==0:
