...
def fn_timer(function):
    ...
    def function_timer(*args, **kwargs):
        ...
        t1 = time.time()
        print ("Total time running : %s h" % (str(((t1 - t0) / 3600))))
        return result
...
def parser_paramter():
    ...
    parser.add_argument("-ce", "--char_emb", help="the char embedding file", default=None) #default ='emb/query.word.300d.vec'
    parser.add_argument("-xpu", "--xpu", help="the configure of gpu or cpu, the default is /gpu:0", default="/gpu:1",
                        type=str)
    
    parser.add_argument("-lr", "--learning_rate", help="the learning rate, the default is 0.001", default=0.0005,
                        type=float)
    ...
    parser.add_argument("-hd", "--hidden_dim", help="the hidden layer dim, the default is 200", default=300, type=int)
    parser.add_argument("-nf", "--num_filter", help="the num filter dim, the default is 100", default=100, type=int)
    ...
    parser.add_argument("-dr", "--dropout_rate", help="the learning rate, the default is 0.5", default=0.5, type=float)
    ...
    return (model_type, name, train_path, dev_path, save_path, fine_tune, num_epochs, emb_path, model_type,
            xpu_config, learning_rate, emb_dim, hidden_dim, num_filter, num_layers,
            num_steps, dropout_rate, val_ratio, batch_size)


@fn_timer
def model_train():
    (model_type, name, train_path, dev_path, save_path, fine_tune, num_epochs, emb_path,model_type,
     xpu_config, learning_rate, emb_dim, hidden_dim, num_filter, num_layers,
    ...
    with tf.Session(config=config) as sess:
        with tf.device(xpu_config):
            ...
            with tf.variable_scope(name, reuse=None, initializer=initializer):
                model = Model(model_type=model_type, num_chars=num_chars, num_classes=num_classes, num_steps=num_steps,
                                   emb_dim=emb_dim, hidden_dim=hidden_dim, num_filter=num_filter, num_layers=num_layers,
            ...
            model.train(sess, char2id, label2id, num_steps, batch_size, os.path.join(save_path, "model"), train_path, dev_path)
